...
def get_top_hit_per_gene(QTL_df):
    ...
    for idx, i in enumerate(unique_gene_list):
        ...
        top_hit_per_current_gene=QTL_df_current_gene[QTL_df_current_gene['p_value'] == min(QTL_df_current_gene['p_value'])].iloc[0,:]
...
def get_top_hit_per_region(QTL_df):
    ...
    for idx, i in enumerate(unique_region_list):
        ...
        # get first line if multiple hits
        top_hit_per_current_region=QTL_df_current_region[QTL_df_current_region['p_value'] == min(QTL_df_current_region['p_value'])].iloc[0,:]
...
def main():
    ...
    if args.top_hit_per_region is True:
        ...
        input_QTL_df=get_top_hit_per_region(input_QTL_df)
    
    if args.top_hit_per_gene is True:
        # filter for only top hit for each gene
        input_QTL_df=get_top_hit_per_gene(input_QTL_df)
